...
class Database:
	...
	def insert_data(self, table_name, data):
		try:
			...
			self.conn.commit()
			return True, f"{self.c.rowcount} rows inserted successfully"
		except sqlite3.Error as e:
	...
	def update_data(self, table_name, set_clause, where_clause):
		try:
			...
			self.conn.commit()
			return True, f"{self.c.rowcount} rows updated successfully"
		except sqlite3.Error as e:
	...
	def delete_data(self, table_name, where_clause):
		try:
			...
			self.conn.commit()
			return True, f"{self.c.rowcount} rows deleted successfully"
		except sqlite3.Error as e:
	...
	def drop_table(self, table_name):
		try:
			...
			return False, f"Error dropping table: {e}"
	...
	def view_data(self, table_name, columns='*', where_clause=None):
		try:
			...
			return False, f"Error viewing data: {e}"
